            ...
            success: function (data) {
                if (data.status === "200") {
                    ...
                    data.data.forEach(function (item, index) {
                        result.append('<tr>');
                        result.append('<td class="button imageTable" ><a href="confirmContract.jsp?contractID=' + item.id + '">' + item.id + '</a></td>');
...
<script>
    function searchId() {
        window.location.href = "confirmContract.jsp?contractID=" + $('#contract').val();
    }
    function Login() {
